...
DWORD Func2 (LPVOID param)
{
    ...
    if (!strcmp(s, "cs"))
        ...
        SetEvent(event); //将event重新设置为signaled状态，相反的，ResetEvent则是设置为non状态
    count++;
    return 0;
}
...
int main()
{
    ...
    //线程的创建和等待
    //创建线程(还可用_beginthreadex)，参数分别为：线程安全信息、栈大小(0为默认大小)、函数指针、函数参数、指定创建后行为(0为立即执行)、线程ID的地址(ID不同于句柄，句柄唯一于进程，而ID唯一于整个内核)。成功时返回句柄，失败返回NULL
    ...
    printf("thread state: %s, tid: %lu\n", (dw==WAIT_OBJECT_0) ? "signaled" : "non-signaled", tid);
    ...
    printf("a = %lld, count = %d, dw = %d\n", a, count, dw);

    //测试内核同步中的mutex
    param_str = "mutex";
    mutex = CreateMutex(NULL, FALSE, NULL); //创建mutex对象，参数为：安全信息设置、是否获取锁(为真则直接锁住，为假则不锁)、互斥对象的名称
    ...
    printf("a = %lld, count = %d\n", a, count);

    //测试内核同步中的sem
    param_str = "sem";
    sem = CreateSemaphore(NULL, 1, 1, NULL); //创建sem对象，参数为：安全信息设置、信号量的初始值、信号量的最大值、互斥对象的名称
    for (int i = 0; i<HD_COUNT; i++)
    {
        hds[i] = CreateThread(NULL, 0, Func2, (LPVOID)param_str, 0, NULL);
    }
    ...
    param_str = "event";
    event = CreateEvent(NULL, FALSE, TRUE, NULL); //创建event对象，参数为：安全信息设置、是否为manual模式(真为manual，假为auto，这里需要所有线程同步，所以用手动模式)、是否为signaled状态、对象名称
    for (int i = 0; i<HD_COUNT; i++)
    {
        hds[i] = CreateThread(NULL, 0, Func2, (LPVOID)param_str, 0, NULL);
    }
