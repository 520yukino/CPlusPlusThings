...
int main(int argc, char *args[])
{
    ...
    memset(&seraddr, 0, sizeof(seraddr)); //可以设置seraddr的其它无用位为0，sockaddr结构是通用套接字地址结构，而sockaddr_in是ipv4独有的，其中的sin_zero字段是占位段
    seraddr.sin_family = AF_INET; //地址族，本质上和PF_INET没有区别
    seraddr.sin_addr.S_un.S_addr = inet_addr("192.168.0.104");//htonl(INADDR_ANY); //将本机(host)cpu解析数据的方式小端序转为网络(network)传输统一使用的大端序，INADDR_ANY为自动分配IP，若只有一个网卡则可以这么做
    seraddr.sin_port = htons(atoi(args[1])); //l为long，表IP；s为short，表端口，端口号实则是为了区分服务器端的套接字
    if (bind(sersock, (SOCKADDR *)(&seraddr), sizeof(seraddr)) == SOCKET_ERROR) //绑定套接字地址信息，注意需要转换地址类型为通用类型。绑定时此地址和端口不能处于占用状态(如处于TIME_WAIT)，否则无法绑定
        ErrorPuts("bind() failed!");
    if (listen(sersock, 3) == SOCKET_ERROR) //转为可连接状态，之后客户端的connect可加入等待队列，参数2为队列最大数，即最多能有n个客户端等待accept，1个正在连接，n+1个最大客户端数，-1表示不限个数
    ...
    {
        mc = 0;
        if ((clisock = accept(sersock, (SOCKADDR *)(&cliaddr), &szcliaddr)) == INVALID_SOCKET) //取出队列中的客户，如果没有就阻塞接受客户端的连接请求，参数2、3并不是必须的，如果不需要地址信息可以传NULL
        ...
        //主动断开连接的一方在关闭过程中的第3次握手后会保持2*MSL时长的TIME_WAIT状态，以应对ACK回应丢失的问题(被动方没有得到ACK则会不断重发FIN，这会大大影响被动方)，等待2个MSL的原因是被动方的重发间隔+FIN信息的传输这2个操作的最长时间
        //在win下，服务端同一端口的使用不会受TIME_WAIT的影响，客户端则会导致端口占用进而connect失败；但在linux下，两端都会受影响
        printf("Server: closed client %d\n", i);
    }

    closesocket(sersock); //主动断开的一方正常情况下最终会处于TIME_WAIT状态等待2MSL再CLOSE。
